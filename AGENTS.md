...
Optional visualizer (demo only): `go run ./visualizer --no-browser visualizer/scenarios/showcase.json` serves a browser UI on `:8080` and launches its own 5-node cluster on ports 8001-8005/9001-9005. Gotchas: it auto-opens a browser unless `--no-browser` is passed, and it calls `KillPorts` on 8001-8005/9001-9005 at startup — so do not run it alongside a manually started cluster on those ports.
